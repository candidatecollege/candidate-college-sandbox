...
const FormCategory: React.FC<any> = ({ onClose, isEdit, category }) => {
  ...
  const handleUploadCategory = async (e: any) => {
    ...
    try {
        const response = await axios.post('https://resource.candidatecollegeind.com/api/event/categories', formData, {
            headers: {
      ...
      onClose()
        ...
        if (isAxiosError(error) && error.response && error.response.status === 401) {
          Swal.fire({
            ...
            timerProgressBar: true,
            icon: 'error',
            title: 'Failed to edit category, unauthenticated!',
          })
        } else {
          Swal.fire({
            toast: true,
  ...
  const handleEditCategory = async (e: any) => {
    ...
    try {
        const response = await axios.post(`https://resource.candidatecollegeind.com/api/event/categories/${category.category.id}`, formData, {
            headers: {
                Authorization: `Bearer ${storedToken}`,
                'Content-Type': 'mulipart/form-data',
            },
