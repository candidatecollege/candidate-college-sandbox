...
const ListCategoryItem: React.FC<any> = ({ category, handleDeleteCategory, isLoading, onEdit }) => {
  return (
    <>
        {
            isLoading ?
        ...
        <li className="border-gray-400 flex flex-row mb-2 w-[50%] relative items-center">
            ...
            <div className="flex flex-row gap-2 absolute right-2">
                <button type="button" onClick={(e) => onEdit(category)} className='bg-secondary rounded-lg flex items-center justify-center p-2 duration-700 transition-all'>
                    <PenIcon size={'24'} color={'#1f1f1f'} />
                </button>
                <button type="button" onClick={(e) => handleDeleteCategory(category.id)} className='bg-red-600 rounded-lg flex items-center justify-center p-2 duration-700 transition-all'>
