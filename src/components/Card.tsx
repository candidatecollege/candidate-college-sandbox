...
const Card: React.FC<any> = ({ data, isLoading, onDelete }) => {
  return (
    <>
        {
            isLoading ? 
            ...
            <div className={`flex-col gap-2 mb-4 md:gap-2 flex relative`}>
                <div className="w-full h-full md:w-full md:h-[178px] rounded-xl object-cover relative">
                    <div className="flex flex-row gap-2 absolute top-2 right-2">
                        <Link href={`/datas/edit/${data.slug}`} title={data.title} about={data.title} className='bg-secondary rounded-lg flex items-center justify-center p-2 duration-700 transition-all'>
